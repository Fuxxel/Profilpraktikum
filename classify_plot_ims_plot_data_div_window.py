...
file_with_plot_data = f"classify_ims_run_{run}_bearing_{lager}.npy"
...
window_size = 20
w = windowed_data(preds, 1, window_size)
stds = np.std(w, 1)
...
plt.title(f"Classification Run {run} Bearing {lager}")
plt.xlabel("Date")
plt.ylabel("Predicted Bearing Health")
plt.tight_layout()
plt.savefig(f"classify_plot_ims_div_20_run_{run}_bearing_{lager}.png")