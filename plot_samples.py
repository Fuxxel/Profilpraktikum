...
def main():
	...
	for latent, window_size in product(latents, window_sizes):
		...
		model_weights = "/scratch/ws/s0403431-profilprojekt/trained_models/Lager4/reduced_model/sensor_0/ws{}/nc1/fs3/skip_first_0/lat_{}/cnn_model.h5".format(window_size, latent)
		
		filter_num = 8 // latent if reduced_model else 64 // latent
		...
		save_path = "reconstruction_plots/Lager4/reduced_model/ws{}/lat_{}/".format(window_size, latent)
		...
		for file in mat_files:
			...
			for i in range(25):
				...
				predicted_sample = np.squeeze(model.predict_on_batch(sample)[0][0], -1)
				mse = np.mean(np.square(original - predicted_sample))
				prediction_data.append((mse, original, predicted_sample))

			prediction_data = sorted(prediction_data, key=lambda x: x[0])
			for i, (mse, original, predicted_sample) in enumerate(prediction_data):
