...
class CNN_Test_DataLoader(tf.keras.utils.Sequence):
	def __init__(self, data_folder, batch_size, sensor, sample_length=1024, normalize_then_split=False):
	...
	def __create_batch_indices(self):
		...
		self.batch_size = min(len(self.batch_indices), (128*4096)//self.sample_length)
	...
	def on_epoch_start(self):
		self.warning = False
		# Load new file from datafolder into memory 
		self.__load_next_file_into_memory()
		self.__create_batch_indices()

	def on_epoch_end(self):
		if self.warning == True:
			print("WARNING: Called on_epoch_end consecutively without calling on_epoch_start in between.")
	...
	def __load_next_file_into_memory(self):
		...
		self.current_timeseries = loadmat(file_to_load)["Data"][..., self.sensor, np.newaxis] 
		if self.normalize_then_split:
			self.current_timeseries = self.__normalize_sample(self.current_timeseries)
...
class CNN_DataLoader(tf.keras.utils.Sequence):
	def __init__(self, data_folder, batch_size, sensor, sample_length, skip_files=0, normalize_then_split=False):
		self.data_folder = data_folder
...
class LSTM_DataLoader(tf.keras.utils.Sequence):
	def __init__(self, data_folder, batch_size, sensor, sample_length, skip_files=0, normalize_then_split=False):
		self.data_folder = data_folder
		self.skip_ahead = sample_length // 2
		self.batch_size = batch_size
...
class LSTM_Test_DataLoader(tf.keras.utils.Sequence):
	def __init__(self, data_folder, batch_size, sensor, sample_length, skip_files=0, normalize_then_split=False):
		self.data_folder = data_folder
		self.skip_ahead = sample_length // 2
		self.batch_size = batch_size
