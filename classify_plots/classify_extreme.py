...
def main(args):
    ...
    # output_path = os.path.join(args.save_path, str(args.offset))
    # os.makedirs(output_path, exist_ok=False)

    roi_list = load_txt_list("roi/Lager4.txt")

    complete_data = expand_with_offset(roi_list, 0)

    train_data = complete_data[:60] + complete_data[-15:]
    test_data = complete_data[60:-15]
    ...
    with torch. no_grad():
        for i, (data_file, category) in enumerate(test_data):
            ...
            print(f"\rEvaluating {i + 1}/{len(test_data)}", end="")
    ...
    plt.xlabel("Date")
    plt.title("Classify Extreme")
    plt.ylabel("Confidence")
    plt.savefig("classify_extreme_plot.png")
    

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # parser.add_argument("-o", "--offset", type=int, default=0)
    # parser.add_argument("-s", "--save_path", type=str, default="classify_results")
