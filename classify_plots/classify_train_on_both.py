...
def main():
    ...
    train_data = train_data_bearing_5 + train_list_bearing_4
    val_data = val_data_bearing_5 + val_list_bearing_4

    print("Loading training data bearing 5...")
    train_data_loader = DataLoader(train_data_bearing_5, batch_size=1024, sensor=0, sample_length=1024)
    print("Loading validation data bearing 5...")
    val_data_loader = DataLoader(val_data_bearing_5, batch_size=1024, sensor=0, sample_length=1024)
    ...
    if not os.path.exists("classifier_train_on_all.p"):
        ...
        torch.save(network.state_dict(), "classifier_train_on_all.p")
    else:
        network = torch.load("classifier_train_on_all.p")
    ...
    np.save("classify_train_on_both_plot_data_on_bearing_5.npy", plot_data_bearing_5)
    ...
    np.save("classify_train_on_both_plot_data_on_bearing_4.npy", plot_data_bearing_4)
