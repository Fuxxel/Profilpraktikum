...
def main():
    ...
    train_list = roi_list[::2]
    test_list = roi_list[1::2]
    val_list = train_list[::4]
    for entry in val_list:
        train_list.remove(entry)

    network = ClassifierNN(input_length=1024).cuda()
    print(network)

    train_data = expand_and_annotate_list(train_list, gt_list)
    test_data = expand_and_annotate_list(test_list, gt_list)
    val_data = expand_and_annotate_list(val_list, gt_list)

    if not os.path.exists("classifier_model_bearing_5.p"):
        print("Loading training data...")
        train_data_loader = DataLoader(train_data, batch_size=1024, sensor=0, sample_length=1024)
        print("Loading validation data...")
        val_data_loader = DataLoader(val_data, batch_size=1024, sensor=0, sample_length=1024)
        ...
        torch.save(network.state_dict(), "classifier_model_bearing_5.p")
    else:
    ...
    with torch. no_grad():
        for i, (data_file, category) in enumerate(test_data):
            if "2019_03_16__14_52_12.mat" in data_file:
                gt_begin_index = i
    ...
    np.save("classify_plot_data_bearing_5.npy", plot_data)
